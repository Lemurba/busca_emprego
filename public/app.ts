import { DEFAULT_AGENT_ALLOWED_DOMAINS, DEFAULT_AGENT_PROMPT, DEFAULT_AGENT_SOURCE_IDS, DEFAULT_JOB_PORTALS } from "../src/agent-defaults.js";
...
function showAgentModal(id?: string) {
  ...
  const selectedSources = new Set(agent?.source_ids ?? DEFAULT_AGENT_SOURCE_IDS);
  ...
  const memory = form?.querySelector<HTMLInputElement>("[data-memory-toggle]");
  const optimization = form?.querySelector<HTMLInputElement>("[data-optimization-toggle]");
  const editScope = form?.querySelector<HTMLInputElement>("[data-edit-scope-toggle]");
  const syncDependencies = () => {
    if (memory && optimization) { if (!memory.checked) optimization.checked = false; optimization.disabled = !memory.checked; }
    form?.querySelectorAll<HTMLInputElement>("[name=editable_fields]").forEach((input) => { input.disabled = !editScope?.checked; });
  };
  memory?.addEventListener("change", syncDependencies);
