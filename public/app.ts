...
function renderSettings(data: Data) {
  ...
  const promptRows = data.agentConfigs.map((agent) => `<article class="agent-setting-row"><div class="agent-setting-main"><div class="agent-setting-title"><div><h3>${escapeHtml(agent.name)}</h3><span>${escapeHtml(agent.role_type)} · versão ${agent.version}</span></div><span class="status-pill ${agent.draft_version_id ? "orange" : "teal"}">${agent.draft_version_id ? "Rascunho" : "Publicado"}</span></div><p>${escapeHtml(agent.prompt || "Sem instruções adicionais.")}</p><div class="agent-setting-meta"><span><i class="state-dot ${agent.memory_enabled ? "on" : ""}"></i>Memória ${agent.memory_enabled ? "ativa" : "desativada"}</span><span><i class="state-dot ${agent.hermes_prompt_optimization ? "on" : ""}"></i>Otimização ${agent.hermes_prompt_optimization ? "assistida" : "manual"}</span><span>${agent.editable_fields.length} campo${agent.editable_fields.length === 1 ? "" : "s"} no escopo</span></div></div><div class="agent-setting-actions">${agent.draft_version_id ? `<button class="primary-button" data-publish-agent="${escapeHtml(agent.id)}" data-version-id="${escapeHtml(agent.draft_version_id)}">Publicar</button>` : ""}<button class="secondary-button" data-edit-agent="${escapeHtml(agent.id)}">Editar</button><button class="text-button" data-agent-history="${escapeHtml(agent.id)}">Histórico</button></div></article>`).join("");
  return `${pageHeading("Sistema", "Configurações", "Preferências do workspace e pontos de integração com o Hermes Agent.")}
    <section class="panel agent-settings-panel"><div class="section-head flush"><div><h2>Prompts dos agentes padrão</h2><p>Ajuste prompt, memória, otimização e escopo. Alterações ficam em rascunho até publicação.</p></div><span class="chip">${data.agentConfigs.length} agente${data.agentConfigs.length === 1 ? "" : "s"}</span></div><div class="agent-settings-list">${promptRows || `<div class="empty-state"><div><strong>Nenhum agente padrão configurado</strong><span>Instale ou sincronize os agentes pelo Hermes.</span></div></div>`}</div></section>
